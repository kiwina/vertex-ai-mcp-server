...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (toolName === "save_generate_project_guidelines") {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully generated guidelines and saved to ${output_path}`,
          },
        ],
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully generated snippet and saved to ${output_path}`,
          },
        ],
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully generated explanation and saved to ${output_path}`,
          },
        ],
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully generated direct answer and saved to ${output_path}`,
          },
        ],
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully generated websearch answer and saved to ${output_path}`,
          },
        ],
      };
      ...
      switch (toolName) {
        ...
        case "edit_file_content": {
          ...
          const validPath = validateWorkspacePath(parsed.path);
          resultText = await applyFileEdits(
            validPath,
            parsed.edits,
            parsed.dryRun
          );
        ...
        default:
          throw new McpError(
            ...
            `Filesystem tool handler not implemented: ${toolName}`
          );
      }
    ...
    if (error instanceof z.ZodError) {
      ...
      console.error(
        `[${new Date().toISOString()}] Unexpected error in tool handler (${toolName}):`,
        error
      );
      throw new McpError(
...
async function main() {
  ...
  console.error(
    `[${new Date().toISOString()}] vertex-ai-mcp-server connecting via stdio...`
  );
  await server.connect(transport);
  console.error(
    `[${new Date().toISOString()}] vertex-ai-mcp-server connected.`
  );
}
