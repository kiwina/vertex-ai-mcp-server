  ...
  paths: z
    .union([
      z
        .string()
        .describe(
          "The path of the file to read (relative to the workspace directory)."
        ),
      z
        .array(z.string())
        .min(1)
        .describe(
          "An array of file paths to read (relative to the workspace directory)."
        ),
    ])
    .describe(
      "A single file path or an array of file paths to read. This tool now handles both single and multiple file reads."
    ),
});
  ...
  buildPrompt: (args: any, modelId: string) => {
    ...
    if (!parsed.success) {
      ...
      throw new McpError(
        ...
        `Invalid arguments for read_file_content: ${parsed.error}`
      );
    }
